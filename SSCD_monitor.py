...
def metrics(data: pd.DataFrame):
    ...
    if data.empty:
        ...
        #first create a list of initial and evolving keys for the bar graph 
        ...
        ###############Add the aggregated bar and time graph##################
        # These will fetch all the test monitor results for the given deployable model and produce a time line and bar graph for all the MTRs
        ...
        bar_graph_data={}
        bar_graph_data.update(bar_graph_data) #append the data from this date
        for i in range(len(previous_bar_graph_data)):
            bar_graph_data.update(previous_bar_graph_data[i])
         #update the bar graph with the old data appended to the current data
        final_bar_graph=dict(title="SSCD Metrics Bar Chart",x_axis_label="X Axis",y_axis_label="Y Axis",rotated=True,data={PREDICTION_DATE_COLUMN:vals_list},categories=keys_all)
        print(final_bar_graph)
